...
- (BOOL)readBOOT
{
    ...
    //self.isValid = self.isValid & ([reader readByte] == 0x4B);
    NSLog(@"Instruction %02x", [reader readByte]); // DOS 1.0 4c ; DOS 2.0 4b
    
    self.contAddress = [reader readWord];
    self.files = [reader readByte];
    self.drives = [reader readByte]; // TODO: DECODE
    [reader readByte];
    self.bootEndAddress = [reader readWord];
    self.isBootable = [reader readByte];
    [reader readByte];
    self.dosAddress = [reader readWord];
    return self.isValid;
}
